...
int main(int argc, char **argv) {
    ...
    Museek::ConfigManager config;
    config.load(config_file);
    //Muconf config(config_file);
    if(!config.hasDomain("shares") || !config.domain("shares").count("database")) {
        cerr << "config file '" << config_file << "' has incomplete or corrupt shares database" << endl;
        exit(-1);
    }
    if(!config.hasDomain("buddy.shares") || ! config.domain("buddy.shares").count("database")) {
    ...
    if (doBuddy) {
        string s = config.get("buddy.shares", dbs);
        state += s + ".state";
    } else {
        string s = config.get("shares", dbs);
        state += s + ".state";
    }
    ...
    if (doBuddy) { 
        folded.save(config.get("buddy.shares", dbs));
    } else {
        folded.save(config.get("shares", dbs));
    }
