...
void Museek::SharesDatabase::load(const string& db, bool add) {
 	NNLOG("newsoul.shares.debug", "loading share database %s", db.c_str());
	mShares.load(db);
	update(add);
}
...
void Museek::SharesDatabase::update(bool add) {
	...
	NNLOG("newsoul.shares.debug", "Updated shares, mNumFolders=%i, mNumFiles=%i", mNumFolders, mNumFiles);
...
void Museek::SharesDatabase::recode(bool add) {
	...
	for(; it != mShares.folders.end(); ++it) {
		...
		if(_redir.empty()) {
 			NNLOG("newsoul.shares.warn", "Couldn't transcode '%s' to network encoding", (*it).first.c_str());
			continue;
		}
		...
		for(; fit != (*it).second->files.end(); ++fit) {
			...
			if(_refn.empty()) {
 				NNLOG("newsoul.shares.warn", "Couldn't transcode '%s' to network encoding", (*fit).first.c_str());
				continue;
			}
...
void Museek::SharesDatabase::update_compressed() {
	...
	if (compress((Bytef *)outbuf, &outbuf_len, (Bytef *)inbuf, i) == Z_OK) {
		for(uint i = 0; i < outbuf_len; i++)
 		...
 		NNLOG("newsoul.shares.warn", "compression error");
...
void Museek::SharesDatabase::search(const string& _query, Folder& result) {
 	NNLOG("newsoul.shares.debug", "sharesdatabase search %s", _query.c_str());
